...
const KuisMoreDrawingControl = () => {
  ...
  const [kkm, setKkm] = useState({ kuis_5: 70 }); // default fallback 70
  ...
  const activeAccordionKey = location.pathname.includes("/belajar/moredrawingcontrol") || location.pathname.includes("/belajar/moredrawingcontrol/kuis")
    ? "4"
    : "0";
      ...
      <div className="p-2">
        ...
        <div className='p-4 mt-5 content' style={{
          ...
          <div style={{paddingLeft:50, paddingRight:50, paddingBottom:50}}>
            ...
            <ul>
              <li>Nilai kelulusan minimum: {kkm.kuis_5}</li>
                ...
                <tbody>
                  {loadingRiwayat ? (
                    ...
                    riwayatNilai.map((item, index) => {
                      const nilai = item.kuis_5 ?? 0;
                      const kkmValue = kkm?.kuis_5 ?? 70; // fallback default
...
export default KuisMoreDrawingControl
