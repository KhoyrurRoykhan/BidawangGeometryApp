...
const SetPosition = () => {
  ...
  const normalizeLine = (line) => line.replace(/\s+/g, '').toLowerCase();
  ...
  const handleSubmit = async () => {
    if (currentQuestion === 1) {
      ...
      setFeedback((prev) => ({ ...prev, question1: isCorrect1
        ? 'Benar! Perintah `setposition` digunakan untuk memindahkan Bidawang ke koordinat tertentu di area gambar, tanpa mengubah arah hadapnya.'
        : 'Salah! Coba perhatikan kembali fungsi dari perintah `setposition`. Fokuskan pada hubungannya dengan posisi, bukan warna, arah, atau ukuran.',
    }));
    } else if (currentQuestion === 2) {
      const isCorrect2 = selectedAnswer2 === 'A';
      setFeedback((prev) => ({ ...prev, question2: isCorrect2
        ? 'Benar! Urutan perintah tersebut membentuk segitiga dengan tiga titik koordinat sesuai yang disebutkan.'
        : 'Salah! Perhatikan urutan koordinat yang digunakan dalam soal. Pastikan titik-titik tersebut membentuk segitiga seperti yang dimaksud.',
    }));
...
const runit = (code, forceReset = false) => {
  ...
  const runitchallanges = (code, forceReset = false, skipCheck = false) => {
    ...
    window.Sk.misceval.asyncToPromise(() =>
      ...
      () => {
        console.log('success');
        if (!skipCheck) {
          checkCodeChallanges(parsedCode); // Kirim parsedCode ke fungsi validasi
        }
      },
      (err) => setOutput((prev) => prev + err.toString())
    );
  ...
  const checkCodeChallanges = (parsedCode) => {
    ...
    for (let i = 0; i < userCodeLines.length; i++) {
      const line = userCodeLines[i].replace(/\s+/g, ''); // Hapus semua spasi
      const valid = validCode[i].replace(/\s+/g, '');     // Hapus semua spasi juga
...
const resetCodeChallanges = () => {
          ...
          >
            ...
            >
              ...
              <ul style={{ color: '#444', lineHeight: '1.8', paddingLeft: '20px' }}>
                ...
                <li>Jika perintah yang dijalankan benar, maka langkah selanjutnya akan terbuka.</li>
              ...
              <Row>
                ...
                <Col xs={12} md={3} style={{ fontSize: '15px', marginBottom: isMobile ? '20px' : '0' }}>
                  ...
                  {[
                    { step: '1a', title: 'Berpindah posisi', code: 'setposition 100 100', description: 'Ubah posisi bidawang yang berada di titik awal menjadi ke titik (100,100) dengan perintah dibawah ini:' },
                    { step: '1b', title: 'Berpindah posisi', code: 'setposition 0 100', description: 'Lanjutkan untuk mengubah posisi bidawang lagi ke titik (0,100) dengan perintah dibawah ini:' }
                  ].map((step, index) => {
  ...
  {currentQuestion === 1 && (
    <Form.Group controlId="question1">
      ...
      {feedback.question1 && (
        <Alert variant={feedback.question1 === 'Benar! Perintah `setposition` digunakan untuk memindahkan Bidawang ke koordinat tertentu di area gambar, tanpa mengubah arah hadapnya.' ? "success" : "danger"} className="mt-3">
          {feedback.question1}
        </Alert>
      )}
    </Form.Group>
  ...
  {currentQuestion === 2 && (
    <Form.Group controlId="question2">
      ...
      {feedback.question2 && (
        <Alert variant={feedback.question2 === 'Benar! Urutan perintah tersebut membentuk segitiga dengan tiga titik koordinat sesuai yang disebutkan.' ? "success" : "danger"} className="mt-3">
          {feedback.question2}
        </Alert>
      )}
    </Form.Group>
  ...
  <div className="text-center mt-4 d-flex justify-content-between">
    ...
    <Button
      ...
      disabled={
        (currentQuestion === 1 && feedback.question1 !== 'Benar! Perintah `setposition` digunakan untuk memindahkan Bidawang ke koordinat tertentu di area gambar, tanpa mengubah arah hadapnya.') ||
