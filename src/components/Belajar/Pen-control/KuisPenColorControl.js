...
const KuisPenColorControl = () => {
  ...
  //kunci halaman
  const [progresBelajar, setProgresBelajar] = useState(21);
  ...
  const [kkm, setKkm] = useState({ kuis_4: 70 }); // default fallback 70
      ...
      <div className="p-2">
        ...
        <div className='p-4 mt-5 content' style={{
          ...
          <div style={{paddingLeft:50, paddingRight:50, paddingBottom:50}}>
            ...
            <ul>
              <li>Nilai kelulusan minimum: {kkm.kuis_4}</li>
                ...
                <tbody>
                  {loadingRiwayat ? (
                    ...
                    riwayatNilai.map((item, index) => {
                      const nilai = item.kuis_4 ?? 0;
                      const kkmValue = kkm?.kuis_4 ?? 70; // fallback default
